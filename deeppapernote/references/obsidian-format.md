...
Required fields:
- `tags`: use a stable hierarchy for the target knowledge base, for example `papers/citrus` and `papers/citrus/<axis>`
...
- `page_type`: page role such as `paper`
- `primary_axis`: the canonical research axis chosen by the knowledge-base schema
- `secondary_tags`: optional auxiliary tags beyond the primary axis
- `zotero_item_key`: Zotero item key
- `zotero_attachment_key`: Zotero attachment key when available
- `zotero_pdf_path`: stable local Zotero attachment path when available
- `source_url`: canonical source URL when available
- `ingest_date`: note ingest date in `YYYY-MM-DD`
- `source_status`: source freshness marker such as `current`
- `related_topics`: topic pages that should link back to this note
- `related_entities`: entity pages that should link back to this note
...
tags:
  - papers/citrus
  - papers/citrus/genetics-genomics
aliases:
  - "Paper Short Name"
date: 2024-05-01
...
zotero_pdf_path: C:\Users\name\Zotero\storage\EFGH5678\paper.pdf
source_url: https://doi.org/10.18653/v1/2024.acl-long.1
