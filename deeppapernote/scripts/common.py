...
SCHEMA_START_MARKER = "<!-- CITRUS_WIKI_SCHEMA_START -->"
SCHEMA_END_MARKER = "<!-- CITRUS_WIKI_SCHEMA_END -->"
ZOTERO_ATTACHMENT_KEY_PATTERN = re.compile(r"^[A-Z0-9]{8}$")
...
def runtime_config() -> dict[str, Any]:
    return {
        ...
        "knowledge_base_schema_relpath": env_config_value(
            "DEEPPAPERNOTE_KB_SCHEMA_RELPATH",
            default="Research/Citrus/AGENTS.md",
        ),
        "wiki_maintainer_ingest_script": env_config_value(
            "DEEPPAPERNOTE_WIKI_MAINTAINER_INGEST_SCRIPT",
            default="",
        ),
...
def configured_knowledge_base_schema_path(config: dict[str, Any]) -> Path | None:
    vault_path = configured_obsidian_vault(config)
    if vault_path is None:
        return None
    relpath = str(config.get("knowledge_base_schema_relpath", "Research/Citrus/AGENTS.md")).strip()
    if not relpath:
        return None
    schema_path = vault_path / Path(relpath)
    if not schema_path.exists() or not schema_path.is_file():
        return None
    return schema_path.resolve()


def load_knowledge_base_schema(config: dict[str, Any]) -> dict[str, Any] | None:
    schema_path = configured_knowledge_base_schema_path(config)
    ...
    if not isinstance(payload, dict):
        return None
    payload.setdefault("schema_path", str(schema_path))
    return payload


def effective_papers_dir(config: dict[str, Any]) -> str:
    schema = load_knowledge_base_schema(config)
    if schema and isinstance(schema.get("papers_dir"), str):
        papers_dir = str(schema.get("papers_dir", "")).strip()
        if papers_dir:
            return papers_dir
    return str(config.get("papers_dir", "Research/Papers")).strip() or "Research/Papers"


def _normalize_schema_axis_list(schema: dict[str, Any]) -> list[dict[str, Any]]:
...
def _normalize_schema_entity_rules(schema: dict[str, Any]) -> list[dict[str, Any]]:
    entity_rules = schema.get("entity_rules", [])
    if not isinstance(entity_rules, list):
        return []
    normalized: list[dict[str, Any]] = []
    for item in entity_rules:
        if not isinstance(item, dict):
            continue
        ...
        normalized.append({"name": name, "keywords": keywords, "aliases": aliases})
    return normalized


def infer_axis_from_schema(schema: dict[str, Any], title: str, abstract: str = "", body: str = "") -> str:
    axes = _normalize_schema_axis_list(schema)
    if not axes:
        return normalize_whitespace(str(schema.get("default_axis", ""))) or "inbox"
    searchable = normalize_whitespace(" ".join(part for part in [title, abstract, body] if part)).lower()
    best_axis = normalize_whitespace(str(schema.get("default_axis", ""))) or axes[-1]["key"]
    best_score = -1
    for axis in axes:
        score = 0
        label = str(axis.get("label", "")).lower()
        if label and label in searchable:
            score += 8
        key = str(axis.get("key", "")).lower()
        if key and key in searchable:
            score += 10
        for token in axis.get("aliases", []) or []:
            lower = str(token).lower()
            if lower and lower in searchable:
                score += 9
        for token in axis.get("keywords", []) or []:
            lower = str(token).lower()
            if lower and lower in searchable:
                score += 6
        if score > best_score:
            best_axis = axis["key"]
            best_score = score
    return best_axis if best_score > 0 else (normalize_whitespace(str(schema.get("default_axis", ""))) or "inbox")


def infer_related_entities_from_schema(schema: dict[str, Any], *, title: str, abstract: str = "", body: str = "") -> list[str]:
    searchable = normalize_whitespace(" ".join(part for part in [title, abstract, body] if part)).lower()
    matched: list[str] = []
    for rule in _normalize_schema_entity_rules(schema):
        score = 0
        if str(rule["name"]).lower() in searchable:
            score += 8
        for token in rule.get("aliases", []) or []:
            if token.lower() in searchable:
                score += 6
        for token in rule.get("keywords", []) or []:
            if token.lower() in searchable:
                score += 4
        if score > 0 and rule["name"] not in matched:
            matched.append(str(rule["name"]))
    return matched


def default_related_topics(schema: dict[str, Any] | None, axis: str) -> list[str]:
    if not axis:
        return []
    return [axis]


def default_wiki_maintainer_ingest_script(config: dict[str, Any]) -> Path | None:
    explicit = normalize_whitespace(str(config.get("wiki_maintainer_ingest_script", "")))
    if explicit:
        path = Path(explicit).expanduser().resolve()
        return path if path.exists() and path.is_file() else None
    candidate = Path(__file__).resolve().parents[2] / "obsidian-wiki-maintainer" / "scripts" / "ingest_maintain.py"
...
def existing_domain_dirs(config: dict[str, Any]) -> list[str]:
    ...
    papers_dir = effective_papers_dir(config)
...
def resolve_domain_subdir(config: dict[str, Any], *, title: str, abstract: str = "", subdir: str = "") -> str:
    ...
    if schema is not None:
        return infer_axis_from_schema(schema, title=title, abstract=abstract)
...
def resolve_obsidian_note_path(
    ...
    *,
    ...
    if subdir:
        ...
        papers_dir_posix = Path(papers_dir).as_posix()
        subdir_posix = subdir_path.as_posix()
        if output_mode == "obsidian" and subdir_posix.startswith(papers_dir_posix):
...
def markdown_frontmatter_split(text: str) -> tuple[dict[str, Any], str]:
    normalized = text or ""
    ...
    if end_index is None:
        return ({}, normalized)
    frontmatter = parse_simple_yaml_block(lines[1:end_index])
...
def parse_simple_yaml_block(lines: list[str]) -> dict[str, Any]:
    ...
    for raw_line in lines:
        ...
        if list_match and current_key:
            ...
            list_items.append(_parse_yaml_scalar(list_match.group(1)))
            continue
        ...
        if remainder is None or remainder == "":
            ...
            parsed[current_key] = _parse_yaml_scalar(remainder)
...
def render_simple_yaml_frontmatter(payload: dict[str, Any]) -> str:
    lines = ["---"]
...
def extract_zotero_attachment_key(record: dict[str, Any]) -> str:
    explicit = normalize_whitespace(str(record.get("zotero_attachment_key", "")))
    if explicit:
        return explicit
    local_pdf_path = normalize_whitespace(str(record.get("local_pdf_path", "")))
    if local_pdf_path:
        parent_name = Path(local_pdf_path).expanduser().parent.name.strip().upper()
        if ZOTERO_ATTACHMENT_KEY_PATTERN.fullmatch(parent_name):
            return parent_name
    return ""


def _paper_tag_prefix(schema: dict[str, Any] | None) -> str:
    if schema and isinstance(schema.get("paper_tag_prefix"), str):
        value = normalize_whitespace(str(schema.get("paper_tag_prefix", "")))
        if value:
            return value
    return "papers"


def normalize_paper_note_frontmatter(
    note_text: str,
    *,
    record: dict[str, Any],
    config: dict[str, Any],
    ...
    schema = load_knowledge_base_schema(config)
    existing, body = markdown_frontmatter_split(note_text)
    body = body or note_text
    axis = normalize_whitespace(str(existing.get("primary_axis", ""))) or normalize_whitespace(resolved_subdir)
    if schema is not None:
        axis = infer_axis_from_schema(schema, title=title, abstract=str(record.get("abstract", "")), body=body)
    paper_tag_prefix = _paper_tag_prefix(schema)
    tags = normalize_string_list(existing.get("tags", []))
    for required_tag in [paper_tag_prefix, f"{paper_tag_prefix}/{axis}"]:
        if required_tag and required_tag not in tags:
            tags.append(required_tag)
    aliases = normalize_string_list(existing.get("aliases", []))
    translated_title = normalize_whitespace(str(record.get("translated_title", "")))
    english_title = normalize_whitespace(str(record.get("english_title", "")))
    for candidate in [english_title, translated_title]:
        if candidate and candidate != title and candidate not in aliases:
            aliases.append(candidate)
    local_pdf_path = normalize_whitespace(str(record.get("local_pdf_path", "")))
    if not local_pdf_path:
        local_pdf_path = normalize_whitespace(str(record.get("pdf_path", "")))
    related_topics = normalize_string_list(existing.get("related_topics", []))
    if not related_topics:
        related_topics = default_related_topics(schema, axis)
    related_entities = normalize_string_list(existing.get("related_entities", []))
    if not related_entities and schema is not None:
        related_entities = infer_related_entities_from_schema(
            schema,
            title=title,
    ...
    normalized = {
        "tags": tags,
        "aliases": aliases,
        "date": normalize_whitespace(str(existing.get("date", ""))) or normalize_whitespace(str(record.get("year", ""))),
        "doi": normalize_whitespace(str(record.get("doi", ""))),
        "page_type": "paper",
        "primary_axis": axis or "inbox",
        "secondary_tags": normalize_string_list(existing.get("secondary_tags", [])),
        "zotero_item_key": normalize_whitespace(str(record.get("zotero_key", ""))),
        "zotero_attachment_key": extract_zotero_attachment_key(record),
        "zotero_pdf_path": local_pdf_path,
        "source_url": normalize_whitespace(str(record.get("source_url", ""))),
        "ingest_date": normalize_whitespace(str(existing.get("ingest_date", ""))) or date.today().isoformat(),
        "source_status": normalize_whitespace(str(existing.get("source_status", ""))) or "current",
        "related_topics": related_topics,
        "related_entities": related_entities,
    }
    frontmatter = render_simple_yaml_frontmatter(normalized)
