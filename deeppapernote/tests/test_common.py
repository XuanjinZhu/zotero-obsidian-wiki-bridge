...
from common import (
    ...
    effective_papers_dir,
    extract_zotero_attachment_key,
...
def test_effective_papers_dir_prefers_schema(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    schema_dir = vault / "Research" / "Citrus"
    schema_dir.mkdir(parents=True)
    (schema_dir / "AGENTS.md").write_text(
        '\n'.join(
            [
                "# schema",
                "<!-- CITRUS_WIKI_SCHEMA_START -->",
                "```json",
                '{"papers_dir":"Research/Citrus/papers","default_axis":"inbox","axes":[{"key":"inbox","label":"Inbox","keywords":[]}]}',
                "```",
                "<!-- CITRUS_WIKI_SCHEMA_END -->",
            ]
        ),
        encoding="utf-8",
    )
    config = {
        "obsidian_vault": str(vault),
        "papers_dir": "Research/Papers",
        "knowledge_base_schema_relpath": "Research/Citrus/AGENTS.md",
        "workspace_output_dir": "DeepPaperNote_output",
    }
    assert effective_papers_dir(config) == "Research/Citrus/papers"


def test_resolve_domain_subdir_uses_schema_axis(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    schema_dir = vault / "Research" / "Citrus"
    schema_dir.mkdir(parents=True)
    (schema_dir / "AGENTS.md").write_text(
        '\n'.join(
            [
                ...
                "<!-- CITRUS_WIKI_SCHEMA_START -->",
                "```json",
                '{"papers_dir":"Research/Citrus/papers","default_axis":"inbox","axes":[{"key":"genetics-genomics","label":"Genetics","keywords":["cytoplasmic","inheritance","mitochondria"]},{"key":"inbox","label":"Inbox","keywords":[]}]}',
                "```",
                "<!-- CITRUS_WIKI_SCHEMA_END -->",
            ]
        ),
    ...
    resolved = resolve_domain_subdir(
        config,
        title="Cytoplasmic inheritance analysis of citrus hybrid populations",
        abstract="Mitochondria and chloroplast inheritance in citrus.",
    )
    assert resolved == "genetics-genomics"


def test_resolve_obsidian_note_path_uses_schema_papers_dir(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    schema_dir = vault / "Research" / "Citrus"
    schema_dir.mkdir(parents=True)
    (schema_dir / "AGENTS.md").write_text(
    ...
    path = resolve_obsidian_note_path(config, title="My Test Paper", subdir="inbox")
    assert path == vault / "Research" / "Citrus" / "papers" / "inbox" / "My_Test_Paper" / "My_Test_Paper.md"


def test_infer_axis_from_schema_falls_back_to_default() -> None:
    schema = {
        "default_axis": "inbox",
        "axes": [
            {"key": "genetics-genomics", "label": "Genetics", "keywords": ["genome"]},
            {"key": "inbox", "label": "Inbox", "keywords": []},
        ],
    }
    assert infer_axis_from_schema(schema, title="Unrelated paper") == "inbox"


def test_extract_zotero_attachment_key_from_local_pdf_path() -> None:
    record = {"local_pdf_path": r"C:\Users\xuanjinzhu\Zotero\storage\RBNDRL42\paper.pdf"}
    assert extract_zotero_attachment_key(record) == "RBNDRL42"


def test_normalize_paper_note_frontmatter_adds_required_fields(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    schema_dir = vault / "Research" / "Citrus"
    schema_dir.mkdir(parents=True)
    (schema_dir / "AGENTS.md").write_text(
        '\n'.join(
            [
                ...
                "<!-- CITRUS_WIKI_SCHEMA_START -->",
                "```json",
                '{"papers_dir":"Research/Citrus/papers","paper_tag_prefix":"papers/citrus","default_axis":"genetics-genomics","entity_rules":[{"name":"细胞质遗传","keywords":["cytoplasmic","inheritance"]},{"name":"柑橘","keywords":["citrus"]}],"axes":[{"key":"genetics-genomics","label":"Genetics","keywords":["cytoplasmic","inheritance"]}]}',
                "```",
                "<!-- CITRUS_WIKI_SCHEMA_END -->",
            ]
        ),
    ...
    record = {
        ...
        "doi": "10.1360/SSV-2025-0276",
        "zotero_key": "PZCL7B39",
        "local_pdf_path": r"C:\Users\xuanjinzhu\Zotero\storage\RBNDRL42\paper.pdf",
        "source_url": "https://example.com/paper",
        "year": "2026",
    }
    normalized = normalize_paper_note_frontmatter(
        note_text,
        record=record,
        config=config,
        title=record["title"],
        resolved_subdir="inbox",
    )
    assert 'page_type: "paper"' in normalized
    assert 'primary_axis: "genetics-genomics"' in normalized
    assert 'zotero_item_key: "PZCL7B39"' in normalized
    assert 'zotero_attachment_key: "RBNDRL42"' in normalized
    assert 'zotero_pdf_path: "C:\\\\Users\\\\xuanjinzhu\\\\Zotero\\\\storage\\\\RBNDRL42\\\\paper.pdf"' in normalized
    assert 'related_topics:' in normalized and '"genetics-genomics"' in normalized
    assert 'related_entities:' in normalized and '"细胞质遗传"' in normalized and '"柑橘"' in normalized


def test_default_wiki_maintainer_ingest_script_prefers_explicit_existing_path(tmp_path: Path) -> None:
    script_path = tmp_path / "ingest_maintain.py"
    script_path.write_text("print('ok')\n", encoding="utf-8")
    config = {"wiki_maintainer_ingest_script": str(script_path)}
    assert default_wiki_maintainer_ingest_script(config) == script_path.resolve()
