...
When the configured Obsidian vault contains `Research/Citrus/AGENTS.md`, treat that file as the active knowledge-base schema:
- use it as the directory and page-type authority
- prefer its research-axis mapping over heuristic keyword folder selection
- preserve its frontmatter contract
- preserve its topic/entity/index/log maintenance expectations
- do not invent a competing folder structure inside the same vault
- after a successful note write, automatically trigger the maintainer ingest step when the companion maintainer skill is installed locally
